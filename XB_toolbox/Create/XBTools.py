...
This script requires the package xbeach_toolbox 

https://github.com/openearth/xbeach-toolbox

it is important to run cells as they are cut to avoid memory overload and to ensure the mask creation is working as it should be
"""
...
class AreaSelector: 
    """
    Is the class the allows to select pixels in map variable
    """
...
def generate_coordinates_inside_polygon(verts):
    """
    allows to extract coordinate in the polygon created with the class above
    """
    path = Path(verts)
...
## save data
sim_path = '/home/tfrn/Documents/Stage_TDV/XBeach/Calibration/2D/2022/Data_more_details/'
...
xgr, zgr = xgrid(x, bathy[400,:], dxmin=5, dxmax=20)
...
plt.tight_layout()
plt.savefig('/home/tfrn/Documents/Stage_TDV/Mémoire/Figures/grille_2d.png')    
#%% 
...
# %% Useless stuff that is needed for xbtools to work
...
xb_setup.set_grid(Xgr, Ygr, zgr, posdwn=1, alfa=280)
