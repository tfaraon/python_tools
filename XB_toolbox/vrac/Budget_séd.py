...
# Script pour calculer les différences de stock sédimentaire entre des entités 
# morpho. Il suffit de lancer le Script et une interface se lance. 

# Le superfolder est le dossier contenant toutes les simulations d'un scénario
# Les données se sauvegardent dans le dossier sélectionné. Le mieux est de 
# choisir le superfolder
...
class XBeachGUI(QWidget):
    ...
    def open_entity_window(self):
        ...
        self.next_button.clicked.connect(self.process_entity)
    ...
    def process_entity(self):
        # Vérifie que les quatre coins ont été définis
        if len(self.corners) != 4:
            QMessageBox.critical(self, "Input Error", "Each entity must have 4 corners.")
            return
        ...
        # Réinitialiser les entrées pour la prochaine entité
        ...
        if len(self.entity_data) == self.num_entities:
            self.finish_entities()

    def finish_entities(self):
        # Vérifie que toutes les entités ont été correctement saisies
        if len(self.entity_data) < self.num_entities:
            QMessageBox.critical(self, "Input Error", "All entities must have 4 corners.")
            return
    ...
    def load_entities(self):
        # Chargement des données des entités depuis un fichier
        filepath, _ = QFileDialog.getOpenFileName(self, "Open Entity File", "", "Text Files (*.txt);;All Files (*)")
        if not filepath:
            return
        ...
        for i, subfolder in enumerate(subfolders):
            ...
            if os.path.exists(xb_output):
                ...
                for entity in zip(self.entity_names, self.entity_data):
                    ...
                    section_start_bed = self.start_bed[min(y1, y2, y3, y4):max(y1, y2, y3, y4), min(x1, x2, x3, x4):max(x1, x2, x3, x4)]
                    section_zb = zb[min(y1, y2, y3, y4):max(y1, y2, y3, y4), min(x1, x2, x3, x4):max(x1, x2, x3, x4)]
